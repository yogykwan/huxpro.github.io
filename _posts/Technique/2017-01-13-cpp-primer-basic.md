...
《C++ Primer（第5版）》读书笔记系列，旨在查漏补缺、重走C++整洁之道。
本篇为第I部分：C++基础，包含全书第1～7章重难点。
...
课后题答案见GitHub仓库[CppPrimer](https://github.com/yogykwan/CppPrimer)。
...
3. 输入输出运算符的计算结果为左侧运算对象，IO操作读写缓冲与程序中的动作无关
...
20. 空指针（nullptr）：不知向任何对象，可以转换成任何其他的指针类型
...
2. string初始化：数字和字符代表string对象内容是将给定字符连续重复给定次数得到的序列
...
6. size_type：string::size_type和vector_type可表示各自类型的长度或下标，无符号整数
...
12. 指针的转换：0或nullptr可转换成任意指针类型，非底层const的指针可以转为void*，底层const的指针可以转为const void*
...
6. 寻找异常处理代码：寻找异常处理代码的过程与函数调用相反，逐层回退直到找到适当类型的catch子句；若为最终未找到则会转到terminate的标准库函数，全无try语句的程序会直接调用terminate
7. 异常安全：确保对象有效，资源无泄漏，程序处于合理状态，等等
8. 异常类：exception中的exception类、stdexcept中定义了几种常见的异常类、new中的bad_alloc、type_info中的bad_cast
...
5. 数组形参：不允许拷贝数组，传递数组时实际上是传递指向首元素的指针；形参也可以是数组引用
...
14. decltype确定返回类型：由于decltype不负责把数组或函数类型转换为对应指针，所以函数声明时要加一个星号
...
20. constexpr函数：能用于常量表达式的函数，但返回值可以是非常量；返回和形参类型都是字面值类型，函数只执行一句return一句；被隐式指定为内联函数；
...
29. 静态数据成员：必须在类的外部定义和初始化，方式和类外部定义成员函数类似；constrexpr类的静态数据成员可以在类内初始化，但若不仅限于编译器替换值的使用场景则必须在类外再重新定义
